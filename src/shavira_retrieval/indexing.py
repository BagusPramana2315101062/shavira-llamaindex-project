...
def make_embed_model(
    ...
    """Membuat embedding model BGE-M3. GPU dipakai otomatis jika tersedia."""
    try:
        import torch
...
def build_bm25_kwargs(
    ...
    Mode default:
    - tanpa custom Indonesian stopwords
    - tanpa stemmer
    - tanpa penentuan bahasa khusus
    """
    ...
    if mode in {"default", "none", "no_stopwords", "no-stopwords"}:
        ...
        print("BM25 mode: default lexical retrieval, no language, no custom stopwords")
        return bm25_kwargs
    ...
    if mode in {"english", "en"}:
        ...
        print("BM25 mode: english stopwords")
        return bm25_kwargs
...
def make_faiss_cache_path(args: argparse.Namespace) -> str:
    """
    Membuat path cache FAISS berdasarkan konfigurasi yang memengaruhi hasil embedding.
    """
    cache_name = (
        f"faiss_"
        f"model_{safe_name(args.model_name)}_"
    ...
    return str(Path(args.index_cache_dir) / cache_name)
...
def build_retrievers(
    ...
    try:
        ...
        print("Mencoba fallback BM25Retriever tanpa parameter language.")
